...
export default function Notes({}: NotesScreenProps): React.ReactElement {
  ...
  const handleNotePress = (note: StudyItem) => {
    ...
    if (note.item_type === 'term') {
      ...
      console.log('[Notes] ExamQuestionModal（実戦問題）を開きます - item_type:', note.item_type);
      if (note.item_type !== 'question') {
        console.error('[Notes] エラー: questionではないデータがExamQuestionModalに渡されそうになりました', note);
  ...
  const handleCreateNote = async (data: {
    ...
    if (!user?.id || !currentProjectId) {
      Alert.alert('エラー', 'ユーザー情報またはプロジェクト情報が取得できませんでした');
      return;
    }
  ...
  const getFilteredNotes = (): StudyItem[] => {
    ...
    filtered = filtered.filter(note => {
      if (!note.item_type || !['term', 'memo', 'question'].includes(note.item_type)) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.filterSection}>
          <View style={styles.filterRow}>
            <Pressable
              style={[styles.typeTab, itemTypeFilter === 'all' && styles.typeTabActive]}
              onPress={() => handleItemTypeChange('all')}
            >
              <Text style={[styles.typeTabText, itemTypeFilter === 'all' && styles.typeTabTextActive]}>
                すべて
              </Text>
            </Pressable>
            <Pressable
              style={[styles.typeTab, itemTypeFilter === 'term' && styles.typeTabActive]}
              onPress={() => handleItemTypeChange('term')}
            >
              <Text style={[styles.typeTabText, itemTypeFilter === 'term' && styles.typeTabTextActive]}>
                📖 用語
              </Text>
            </Pressable>
            <Pressable
              style={[styles.typeTab, itemTypeFilter === 'memo' && styles.typeTabActive]}
              onPress={() => handleItemTypeChange('memo')}
            >
              <Text style={[styles.typeTabText, itemTypeFilter === 'memo' && styles.typeTabTextActive]}>
                📝 メモ
              </Text>
            </Pressable>
            <Pressable
              style={[styles.typeTab, itemTypeFilter === 'question' && styles.typeTabActive]}
              onPress={() => handleItemTypeChange('question')}
            >
              <Text style={[styles.typeTabText, itemTypeFilter === 'question' && styles.typeTabTextActive]}>
                ❓ 問題
              </Text>
            </Pressable>
          </View>
        ...
        <View style={styles.categorySection}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryScroll}>
            ...
            {allCategories.map((category) => (
              ...
              >
                <Text style={[styles.categoryChipText, categoryFilter === category && styles.categoryChipTextActive]}>
                  {category}
                </Text>
              </Pressable>
            ))}
          ...
          {filteredNotes.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>
                {notes.length === 0 ? 'まだノートがありません' : '該当するノートが見つかりません'}
              </Text>
        ...
        <Pressable
          style={styles.fab}
          onPress={() => navigation.navigate('NoteEditor', { itemType: 'memo' })}
